...
### Rule: name the smallest fix, not the fix I'd rather build

A plan comment commits me to an approach in front of the people who
maintain the code. The eval set this week is full of plans that talk
themselves into a rewrite because the one-line fix feels unsatisfying —
"rather than patch just this, I'll fix the whole pipeline properly" is
the tell. I say what change actually resolves the reported defect, and
name anything bigger as a separate idea the maintainer can take or leave,
not as part of what I'm asking them to review.

- Wrong: "Rather than patch just the save path, I'm going to rebuild the
  whole image pipeline while I'm in there."
- Right: "The fix is scoped to the save path in `image.go`; I noticed
  the pipeline has some structural issues too, but that's a separate
  conversation and not part of this change."

### Rule: say what the thread already told me

If a maintainer already pointed at a cause, posted a patch, or said a
direction is too expensive, my plan says so and either builds on it or
states why it doesn't. Silently proposing something unrelated, as if the
thread were empty, reads as not having read it.

- Wrong: (posting a plan that ignores a maintainer's in-progress patch,
  with no mention of it at all)
- Right: "Building on the patch @maintainer posted in commit abc123: my
  plan closes the one case it doesn't yet cover."
...
- Undisclosed AI assistance, on any repo whose policy asks for
  disclosure.
- A plan that reads as a rewrite because I found the minimal fix
  boring, dressed up as "doing it properly."
